...
Two tables, stdlib sqlite3 only:
  - jobs: one row per uploaded document, its hash, processing status,
    (when done) the full result JSON, and the summary counts GET /jobs
    lists (page_count, pages_needing_review, corrections_count) so the
    list never re-parses result JSON or reads corrections from disk.
