...
for f in schema/schema.json schema/doc_demo.json tokens.css ui.css editor.css \
         index.html api.js translit.js editor.js; do
...
if grep -rEn '#[0-9A-Fa-f]{6}([0-9A-Fa-f]{2})?\b' ui.css editor.css index.html api.js translit.js editor.js; then
...
if grep -rin 'purple' tokens.css ui.css editor.css index.html api.js translit.js editor.js schema/; then
  bad "found purple"
else
...
if grep -rEn 'http|cdn' index.html api.js translit.js editor.js ui.css editor.css; then
